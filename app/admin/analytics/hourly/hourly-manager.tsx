...
interface HourlyManagerProps {
  initialOrders: HourlyOrder[];
  selectedDate: string;
}

export default function HourlyManager({
  initialOrders,
  selectedDate,
}: HourlyManagerProps) {
  ...
  const chartData = Array.from({ length: 24 }, (_, i) => {
    const ordersInHour = initialOrders.filter(
      (o) => new Date(o.created_at).getHours() === i,
    );
    return {
      hour: `${i}:00`,
  ...
  return (
    <div className="p-6 space-y-6">
      <input
        ...
        value={selectedDate}
        onChange={(e) =>
          router.push(`/admin/analytics/hourly?date=${e.target.value}`)
        }
      ...
      <div className="h-100 w-full border rounded-2xl p-4 bg-white">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            data={chartData}
            onClick={(state) =>
              state?.activeLabel && setSelectedHour(String(state.activeLabel))
            }
          >
            <XAxis dataKey="hour" />
